...
## 7. Apakšmasīvu summa uz mērķi

### Uzdevums

Dots saraksts `nums` un skaitlis `k`.

Atgriez, cik daudz **nepārtrauktu apakšmasīvu** summā dod tieši `k`.

Uzdevumu risina ar prefix summām un dictionary (`summa -> cik reizes redzēta`).
...
k = 2
# Output: 2
# Paskaidrojums: der apakšmasīvi [1,1] (0..1) un [1,1] (1..2)
```
...
## 11. Pirmais neatkārtojošais simbols

### Uzdevums

Dota virkne `s`.

Atgriez indeksa pozīciju pirmajam simbolam, kas parādās tieši vienu reizi.

Ja tāda nav, atgriez `-1`.
...
## 12. Vai satur gandrīz dublikātu (attālumā k)

### Uzdevums

Dots saraksts `nums` un skaitlis `k`.

Nosaki, vai eksistē divi vienādi elementi `nums[i] == nums[j]`, kuru indeksu starpība ir ne lielāka par `k`.
...
## 13. Garākā secīgo skaitļu virkne

### Uzdevums

Dots veselu skaitļu saraksts `nums`.

Atgriez garumu garākajai virknei, kur skaitļi iet secīgi (piemēram, `5,6,7,8`), neatkarīgi no to secības sākotnējā sarakstā.
...
# Output: 4
# Paskaidrojums: secīgā virkne ir [1,2,3,4]
```

---

## 14. Trūkstošais skaitlis diapazonā

### Uzdevums

Dots saraksts `nums`, kurā ir visi skaitļi no `0` līdz `n`, izņemot vienu.

Atrodi trūkstošo skaitli.
...
## 15. Nulles summas apakšmasīvs

### Uzdevums

Dots veselu skaitļu saraksts `nums`.

Nosaki, vai eksistē nepārtraukts apakšmasīvs ar summu `0`.
